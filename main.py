...
async def show_details(callback: CallbackQuery, state: FSMContext):
    ...
    for q, user_selected in wrongs:
        text = f"❌ *{q['text']}*\n"
        for idx, (opt_text, _) in enumerate(q["options"]):
            mark = "☑️" if idx in user_selected else "🔘"
            text += f"{mark} {opt_text}\n"
        selected_text = [q["options"][i][0] for i in user_selected] if user_selected else ["—"]
        correct_text = [txt for txt, ok in q["options"] if ok]
